...
describe('Test ToDo DAO - save', () => {
    ...
    it('should save todo', async () => {
        ...
        AWS.mock('DynamoDB.DocumentClient', 'put', (params : PutItemInput, callback) => {
            ...
            expect(params.Item).to.have.property('title', 'ToDoText');
            expect(params.Item).to.have.property('status', 'OPEN');
        ...
        expect(todo).to.have.property('title', 'ToDoText');
        expect(todo).to.have.property('status', 'OPEN');
    });
...
describe('Test ToDo DAO - list', () => {
    it('should list todos', async () => {
        ...
        expect(list[0]).to.have.property('title', 'ToDoText');
        expect(list[0]).to.have.property('status', 'OPEN');
    });
    ...
    it('should list todos with paging', async () => {
        ...
        expect(list[0]).to.have.property('id', 'todoId');
        expect(list[0]).to.have.property('title', 'ToDoText1');
        expect(list[0]).to.have.property('status', 'OPEN');

        expect(list[1]).to.have.property('id', 'todoId');
        expect(list[1]).to.have.property('title', 'ToDoText0');
        expect(list[1]).to.have.property('status', 'OPEN');
